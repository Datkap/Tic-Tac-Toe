...
def select_mark(player_1_name, player_2_name):
    ...
    if chosen_mark == 'x':
        player_1_mark = 'x'
        player_2_mark = 'o'
        print(f"{player_1_name} is playing '{player_1_mark}' and {player_2_name} is playing '{player_2_mark}'.")
    else:
        player_1_mark = 'o'
        player_2_mark = 'x'
        print(f"{player_1_name} is playing '{player_1_mark}' and {player_2_name} is playing '{player_2_mark}'.")

    return player_1_mark, player_2_mark
...
def get_coordinates_from_player(board):
    """Ask current player for coordinates where he wants to put his mark."""
...
def put_mark_on_board(board, coords, mark):
    """Puts current player's mark on board. Informs player if the field is taken and asks for new coords."""
    if board[coords] == '.':
        board[coords] = mark
        return (board, True)
    else:
